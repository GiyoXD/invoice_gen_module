...
def process_single_table_sheet(
    ...
    fill_success, next_row_after_footer, _, _, _ = fill_invoice_data(
        worksheet=worksheet,
    ...
    if weight_summary_config.get("enabled"):
        ...
        if processed_tables_data:
            next_row_after_footer = write_grand_total_weight_summary(
                worksheet=worksheet,
    ...
    if summary_data_source and sheet_inner_mapping_rules_dict:
        for map_key, map_rule in sheet_inner_mapping_rules_dict.items():
            if isinstance(map_rule, dict) and 'marker' in map_rule:
                target_cell = find_cell_by_marker(worksheet, map_rule['marker'])
        ...
        for sheet_name in sheets_to_process:
            ...
            if data_source_indicator == "processed_tables_multi":
                ...
                for i, table_key in enumerate(table_keys):
                    ...
                    written_header_info = write_header(
                    ...
                    fill_success, next_row_after_chunk, data_start, data_end, table_pallets = fill_invoice_data(
                        worksheet=worksheet,
                        sheet_name=sheet_name,
                        ...
                        if not is_last_table: 
                            ...
                            if num_cols_spacer > 0:
                                try:
                                    ...
                                    unmerge_row(worksheet, spacer_row, num_cols_spacer) # Ensure clear
                ...
                # ***** ADD GRAND TOTAL ROW (for multi-table summary) *****
                    if num_tables > 1 and last_table==i:
                        ...
                        try:
                            ...
                            footer_row_index = write_footer_row(
                                worksheet=worksheet,
                ...
                if summary_flag and processing_successful and last_table_header_info and args.DAF:
                    ...
                    write_pointer_row = write_summary_rows(
                        worksheet=worksheet,
