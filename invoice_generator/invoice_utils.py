...
def write_grand_total_footer(
    ...
    """
    Simplified wrapper for writing grand total footers using FooterBuilder.
    This is the proper way for processors to create footers without touching builders directly.
    
    Args:
        worksheet: The openpyxl worksheet
        footer_row_num: Row number for the footer
        header_info: Header information dict
        sum_ranges: List of (start_row, end_row) tuples to sum
        footer_config: Footer configuration dict
        pallet_count: Total pallet count
        sheet_config: Full sheet configuration
        DAF_mode: Whether DAF mode is enabled
        
    Returns:
        The next available row number after the footer
    """
    try:
        # Make a copy and ensure it's marked as grand_total type
        footer_cfg_copy = footer_config.copy()
        footer_cfg_copy["type"] = "grand_total"
        ...
        if styling_config:
            try:
                styling_model = StylingConfigModel(**styling_config)
            except Exception as style_err:
        ...
        if result_row > footer_row_num:
            return result_row
        else:
            print("Warning: FooterBuilder returned invalid row number")
            return footer_row_num + 1
            
    except Exception as e:
        print(f"Error in write_grand_total_footer: {e}")
        traceback.print_exc()
        return footer_row_num + 1
...
# --- Fill Row Before Footer ---
        ...
        # --- Fill Footer Row Using FooterBuilder --- (Refactored to use builder)
        if footer_row_final > 0:
            ...
            # Convert styling dict to StylingConfigModel
            styling_model = None
            if sheet_styling_config:
                try:
                    styling_model = StylingConfigModel(**sheet_styling_config)
                except Exception as style_err:
                    print(f"Warning: Could not create StylingConfigModel: {style_err}")
